import { Metadata } from "next";
import { ScrollReveal } from "@/components/animations/ScrollReveal";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import {
  TrendingUp,
  Users,
  Eye,
  BarChart3,
  Megaphone,
  CheckCircle2,
  ArrowRight,
  Mail,
} from "lucide-react";
import { getPublicAdsData } from "@/lib/supabase/services/admin-data";

export const metadata: Metadata = {
  title: "Beriklan",
  description:
    "Beriklan di BeritaAE - Jangkau jutaan pembaca Indonesia dengan berbagai format iklan.",
};

export const dynamic = "force-dynamic";
export const revalidate = 60;

export default async function AdvertisePage() {
  const { placements, settings } = await getPublicAdsData();

  const stats = [
    { icon: Eye, value: "15M+", label: "Pageviews/Bulan" },
    { icon: Users, value: "2.5M+", label: "Unique Visitors" },
    { icon: TrendingUp, value: "8.5 menit", label: "Avg. Time on Site" },
    { icon: BarChart3, value: "4.2", label: "Pages/Session" },
  ];

  const benefits = [
    "Targeting berdasarkan kategori berita",
    "Laporan performa real-time",
    "A/B testing untuk optimasi",
    "Brand safety guaranteed",
    "Support 24/7 dari tim kami",
    "Flexible budget & duration",
  ];

  const formatPrice = (price: number) => {
    return new Intl.NumberFormat("id-ID", {
      style: "currency",
      currency: "IDR",
      minimumFractionDigits: 0,
    }).format(price);
  };

  return (
    <div className="min-h-screen bg-gradient-to-b from-muted/50 to-background">
      {/* Hero */}
      <section className="py-20 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-r from-orange-500/10 via-transparent to-red-500/10" />
        <div className="absolute -top-40 -right-40 w-80 h-80 bg-orange-500/20 rounded-full blur-3xl" />

        <div className="container mx-auto px-4 relative">
          <ScrollReveal>
            <div className="max-w-3xl">
              <Badge className="mb-4 bg-orange-500">Advertising</Badge>
              <h1 className="text-4xl md:text-5xl font-bold mb-6">
                {settings?.page_title || "Jangkau"}{" "}
                <span className="bg-gradient-to-r from-orange-500 to-red-500 bg-clip-text text-transparent">
                  Jutaan Pembaca
                </span>{" "}
                Indonesia
              </h1>
              <p className="text-lg text-muted-foreground mb-8">
                {settings?.page_description ||
                  "Tingkatkan brand awareness dan konversi bisnis Anda dengan beriklan di platform berita terpercaya dengan engagement tertinggi."}
              </p>
              <div className="flex flex-wrap gap-4">
                <Button
                  size="lg"
                  className="gap-2 bg-orange-500 hover:bg-orange-600"
                  asChild
                >
                  <a href="#paket">
                    <Megaphone className="h-5 w-5" />
                    Mulai Beriklan
                  </a>
                </Button>
                <Button size="lg" variant="outline" className="gap-2" asChild>
                  <a
                    href={`mailto:${
                      settings?.contact_email || "iklan@beritaae.com"
                    }`}
                  >
                    <Mail className="h-5 w-5" />
                    Hubungi Sales
                  </a>
                </Button>
              </div>
            </div>
          </ScrollReveal>
        </div>
      </section>

      {/* Stats */}
      <section className="py-12 border-y bg-background">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {stats.map((stat, index) => (
              <ScrollReveal key={stat.label} delay={index * 0.1}>
                <div className="text-center">
                  <stat.icon className="h-8 w-8 mx-auto mb-3 text-orange-500" />
                  <div className="text-3xl font-bold mb-1">{stat.value}</div>
                  <div className="text-sm text-muted-foreground">
                    {stat.label}
                  </div>
                </div>
              </ScrollReveal>
            ))}
          </div>
        </div>
      </section>

      {/* Ad Formats */}
      <section id="paket" className="py-16">
        <div className="container mx-auto px-4">
          <ScrollReveal>
            <div className="text-center mb-12">
              <h2 className="text-3xl font-bold mb-4">Format Iklan</h2>
              <p className="text-muted-foreground max-w-2xl mx-auto">
                Pilih format yang sesuai dengan kebutuhan kampanye Anda
              </p>
            </div>
          </ScrollReveal>

          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {placements.length === 0 ? (
              <div className="col-span-full text-center py-8 text-muted-foreground">
                Paket iklan akan segera tersedia.
              </div>
            ) : (
              placements.map((placement, index) => {
                const isPopular =
                  index === 0 || placement.price_monthly >= 5000000;
                return (
                  <ScrollReveal key={placement.id} delay={index * 0.1}>
                    <Card
                      className={`border-0 shadow-lg hover:shadow-xl transition-all relative ${
                        isPopular ? "ring-2 ring-orange-500" : ""
                      }`}
                    >
                      {isPopular && (
                        <Badge className="absolute -top-3 left-1/2 -translate-x-1/2 bg-orange-500">
                          Popular
                        </Badge>
                      )}
                      <CardContent className="p-6">
                        <h3 className="text-xl font-bold mb-2">
                          {placement.name}
                        </h3>
                        <p className="text-sm text-muted-foreground mb-1">
                          {placement.size}
                        </p>
                        <p className="text-sm text-muted-foreground mb-4">
                          {placement.position}
                        </p>
                        <div className="text-2xl font-bold text-orange-500 mb-4">
                          {formatPrice(placement.price_monthly)}
                          <span className="text-sm font-normal text-muted-foreground">
                            /bulan
                          </span>
                        </div>
                        {placement.price_weekly > 0 && (
                          <p className="text-sm text-muted-foreground mb-4">
                            atau {formatPrice(placement.price_weekly)}/minggu
                          </p>
                        )}
                        <Button
                          className="w-full"
                          variant={isPopular ? "default" : "outline"}
                          asChild
                        >
                          <a
                            href={`mailto:${
                              settings?.contact_email || "iklan@beritaae.com"
                            }?subject=Inquiry: ${placement.name}`}
                          >
                            Pilih Format
                          </a>
                        </Button>
                      </CardContent>
                    </Card>
                  </ScrollReveal>
                );
              })
            )}
          </div>
        </div>
      </section>

      {/* Benefits */}
      <section className="py-16 bg-muted/30">
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <ScrollReveal>
              <div>
                <h2 className="text-3xl font-bold mb-6">Mengapa BeritaAE?</h2>
                <ul className="space-y-4">
                  {benefits.map((benefit, index) => (
                    <li key={index} className="flex items-center gap-3">
                      <CheckCircle2 className="h-5 w-5 text-green-500 shrink-0" />
                      <span>{benefit}</span>
                    </li>
                  ))}
                </ul>
              </div>
            </ScrollReveal>

            <ScrollReveal delay={0.2}>
              <Card className="border-0 shadow-2xl bg-gradient-to-br from-orange-500 to-red-500 text-white">
                <CardContent className="p-8">
                  <h3 className="text-2xl font-bold mb-4">Konsultasi Gratis</h3>
                  <p className="text-white/80 mb-6">
                    Tim sales kami siap membantu merencanakan kampanye iklan
                    yang efektif untuk bisnis Anda.
                  </p>
                  <div className="space-y-3">
                    {settings?.contact_email && (
                      <p className="text-sm">
                        <strong>Email:</strong> {settings.contact_email}
                      </p>
                    )}
                    {settings?.whatsapp && (
                      <p className="text-sm">
                        <strong>WhatsApp:</strong> {settings.whatsapp}
                      </p>
                    )}
                  </div>
                  <Button
                    size="lg"
                    variant="secondary"
                    className="w-full gap-2 mt-6"
                    asChild
                  >
                    <a
                      href={`mailto:${
                        settings?.contact_email || "iklan@beritaae.com"
                      }`}
                    >
                      Hubungi Kami <ArrowRight className="h-4 w-4" />
                    </a>
                  </Button>
                </CardContent>
              </Card>
            </ScrollReveal>
          </div>
        </div>
      </section>
    </div>
  );
}
