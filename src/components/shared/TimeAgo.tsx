...
export function TimeAgo({ date, className = "", short = false }: TimeAgoProps) {
  ...
  // Fallback to "baru saja" if date is invalid
  if (!isValidDate) {
    return <span className={className}>{short ? "baru" : "baru saja"}</span>;
  }
  ...
  const cleanTimeAgo = timeAgo.replace(/sekitar /i, "").replace(/yang /i, "");
